<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="医护类型" prop="yihuleixing">
          <el-select v-model="ruleForm.yihuleixing" placeholder="请选择医护类型">
            <el-option
                v-for="(item,index) in yihuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="医院" prop="yiyuan">
          <el-select v-model="ruleForm.yiyuan" placeholder="请选择医院">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="医院" prop="yiyuan">
	      <el-input v-model="ruleForm.yiyuan"
                placeholder="医院" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yihuleixing/yihuleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yihuleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yihuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yihuxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
