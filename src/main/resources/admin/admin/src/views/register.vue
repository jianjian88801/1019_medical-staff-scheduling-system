  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(208, 163, 140, 0.53);borderRadius:10px">
        <h1 class="h1" style="color:rgba(255, 255, 255, 1);fontSize:28px;">医护人员排班系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			<!-- <div v-if="tableName=='yihu'" class="input-group">
			   <div class="label">工号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.gonghao" class="input" type="text" placeholder="工号">
			   </div>
			 </div> -->
			<el-form-item label="工号" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="姓名" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="年龄" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="手机号" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='yihu'">
			...
			<el-form-item label="身份证" class="input" v-if="tableName=='yihu'">
...
export default {
  ...
  methods: {
    ...
    // 注册
    login() {
      if((!this.ruleForm.gonghao) && `yihu` == this.tableName){
        this.$message.error(`工号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `yihu` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if((!this.ruleForm.xingming) && `yihu` == this.tableName){
        this.$message.error(`姓名不能为空`);
        return
      }
      if(`yihu` == this.tableName && this.ruleForm.nianling&&(!this.$validate.isIntNumer(this.ruleForm.nianling))){
        this.$message.error(`年龄应输入整数`);
        return
      }
      if(`yihu` == this.tableName && this.ruleForm.shoujihao&&(!this.$validate.isMobile(this.ruleForm.shoujihao))){
        this.$message.error(`手机号应输入手机格式`);
        return
      }
      if(`yihu` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if(`yihu` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
